...
create trigger set_transport_orders_updated_at before update on public.transport_orders for each row execute function public.set_updated_at();
...
for update using (public.has_company_role(id, array['owner','admin']))
...
create policy company_users_owner_admin_insert on public.company_users
...
create policy company_users_owner_admin_update on public.company_users
for update using (public.has_company_role(company_id, array['owner','admin']))
...
create policy company_users_owner_admin_delete on public.company_users
...
for all using (public.has_company_role(company_id, array['owner','admin','dispatcher']))
...
create policy transport_orders_member_select on public.transport_orders
for select using (public.is_company_member(company_id));

drop policy if exists transport_orders_manage on public.transport_orders;
create policy transport_orders_manage on public.transport_orders
for all using (public.has_company_role(company_id, array['owner','admin','dispatcher']))
with check (public.has_company_role(company_id, array['owner','admin','dispatcher']));
...
for all using (public.has_company_role(company_id, array['owner','admin','dispatcher','driver']))
...
for all using (public.has_company_role(company_id, array['owner','admin','accountant']))
...
for all using (public.has_company_role(company_id, array['owner','admin','dispatcher','accountant']))
...
comment on policy trips_manage on public.trips is 'Starter policy. Tighten driver access further before production.';
