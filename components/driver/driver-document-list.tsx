...
export function DriverDocumentList({ documents }: { documents: DocumentItem[] }) {
  if (documents.length === 0) {
    return (
      <Card className="border-dashed border-slate-300 bg-white/90">
        <CardContent className="p-6 text-sm text-slate-600">No trip documents uploaded yet. Photos and POD files will appear here after the first field upload.</CardContent>
      </Card>
    )
  }
