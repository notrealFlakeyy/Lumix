...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (context.membership.role !== 'driver' || !context.membership.enabledModules.includes('transport')) {
    return NextResponse.json({ error: 'Trip checkpoints are only available to driver users.' }, { status: 403 })
  }
  ...
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Unable to capture checkpoint.' }, { status: 400 })
  }
}
