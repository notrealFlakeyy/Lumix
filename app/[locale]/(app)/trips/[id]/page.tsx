    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        <Card >
          ...
          <CardContent className="space-y-3 text-sm text-muted-foreground">
            <div className="flex items-center gap-3"><span className="font-medium text-foreground">Status:</span> <TripStatusBadge status={trip.status as any} /></div>
            ...
            <div><span className="font-medium text-foreground">Linked Order:</span> {order ? <Link href={`/orders/${order.id}`}>{order.order_number}</Link> : 'No linked order'}</div>
            <div><span className="font-medium text-foreground">Linked Invoice:</span> {invoice ? <Link href={`/invoices/${invoice.id}`}>{invoice.invoice_number}</Link> : 'No linked invoice'}</div>
          </CardContent>
        </Card>
        ...
        <Card >
          ...
          <CardContent className="space-y-3 text-sm text-muted-foreground">
            <div><span className="font-medium text-foreground">Start Time:</span> {formatDateTime(trip.start_time)}</div>
            <div><span className="font-medium text-foreground">End Time:</span> {formatDateTime(trip.end_time)}</div>
            <div><span className="font-medium text-foreground">Waiting Time:</span> {trip.waiting_time_minutes} min</div>
            <div><span className="font-medium text-foreground">Start KM:</span> {trip.start_km ? `${toDisplayNumber(trip.start_km)} km` : '-'}</div>
            <div><span className="font-medium text-foreground">End KM:</span> {trip.end_km ? `${toDisplayNumber(trip.end_km)} km` : '-'}</div>
            <div><span className="font-medium text-foreground">Distance KM:</span> {trip.distance_km ? `${toNumber(trip.distance_km)} km` : '-'}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        <Card >
          ...
          <CardContent className="text-sm text-muted-foreground">{trip.delivery_confirmation ?? 'No delivery confirmation captured yet.'}</CardContent>
        </Card>
        ...
        <Card >
          ...
          <CardContent className="text-sm text-muted-foreground">{trip.notes ?? 'No notes recorded.'}</CardContent>
        </Card>
      </div>
