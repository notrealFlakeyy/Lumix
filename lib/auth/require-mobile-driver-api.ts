...
import { requireMobileApiCompany } from '@/lib/auth/require-mobile-api-company'
...
async function resolveCurrentWorkforceEmployee(
  ...
  supabase: NonNullable<Awaited<ReturnType<typeof requireMobileApiCompany>>>['supabase'],
) {
...
type RequireMobileDriverApiOptions = {
  previewDriverId?: string | null
  requireTimeModule?: boolean
}

export async function requireMobileDriverApi(request: Request, options: RequireMobileDriverApiOptions = {}) {
  const context = await requireMobileApiCompany(request)
  if (!context) {
