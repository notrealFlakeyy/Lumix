...
export async function listOrders(companyId: string, client?: DbClient, branchIds?: readonly string[] | null) {
  const supabase = await getDbClient(client)
  const branchScope = normalizeBranchScope(branchIds)
  let ordersQuery = supabase.from('transport_orders').select('*').eq('company_id', companyId).order('created_at', { ascending: false })
  if (branchScope) {
    ordersQuery = ordersQuery.in('branch_id', branchScope)
  }

  const [{ data: orders }, { data: customers }, { data: vehicles }, { data: drivers }, { data: branches }] = await Promise.all([
    ordersQuery,
  ...
  return ((orders ?? []) as TableRow<'transport_orders'>[]).map((order) => ({
    ...order,
    branch_name: order.branch_id ? branchMap.get(order.branch_id)?.name ?? 'Unknown branch' : 'No branch',
