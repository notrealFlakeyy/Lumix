...
export async function listDrivers(
  ...
  const [{ data: drivers, count }, { data: branches }] = await Promise.all([
    query,
    supabase.from('branches').select('id, name').eq('company_id', companyId),
  ])
  const branchMap = new Map((branches ?? []).map((branch) => [branch.id, branch.name]))

  const data = ((drivers ?? []) as TableRow<'drivers'>[]).map((driver) => ({
    ...driver,
    branch_name: driver.branch_id ? (branchMap.get(driver.branch_id) ?? '—') : '—',
  }))
